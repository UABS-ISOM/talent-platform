...
watch(
  project,
  () => {
    newBid.value = project.value?.groupProject?.bid ?? "";
  },
  { immediate: true }
);
...
const { mutate: editGroupProject } = useMutation(
  graphql(`
    mutation EditGroupProjectMutation($input: EditGroupProjectInput!) {
 ...
 */
const changeStatus = async (bidStatus: CourseProjectBidStatusEnum) => {
